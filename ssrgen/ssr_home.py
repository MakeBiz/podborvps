...
if os.path.exists(idxp):
    for e in json.load(open(idxp, encoding="utf-8")):
        ...
        if os.path.exists(p):
            ...
            title = re.sub(r"\s*\(ВПС\)", "", title)  # в чипах-подборках держим чистый заголовок, без кириллического дубля
...
parts.append('<h1>Подберём VPS (ВПС) под вашу задачу</h1>')
parts.append('<p>ПодборVPS — независимый калькулятор, рейтинг и справочный каталог VPS (ВПС). Укажите задачу, нужные ресурсы, географию и бюджет — и получите отсортированный под вашу задачу список и сравнение виртуальных серверов российских и зарубежных провайдеров. Партнёрские ссылки не влияют на порядок подбора.</p>')
